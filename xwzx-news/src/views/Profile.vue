  ...
  <div class="profile-page">
    ...
    <div class="profile-container">
      <van-cell-group inset class="avatar-group">
        <van-cell title="头像" center is-link @click="showAvatarPicker">
          <template #right-icon>
            <van-image
              round
              width="60"
              height="60"
              :src="userStore.getFullAvatarUrl || 'https://fastly.jsdelivr.net/npm/@vant/assets/cat.jpeg'"
            />
          </template>
        </van-cell>
      </van-cell-group>
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        showFailToast((result && result.message) || '头像上传失败');
      }
    } catch (error) {
...
const showUrlInputDialog = () => {
  tempAvatarUrl.value = userInfo.value?.avatar || '';
  ...
  showDialog({
    ...
    message: h('div', { style: 'text-align: left; padding: 10px 0;' }, [
      h('div', { style: 'margin-bottom: 15px;' }, [
        ...
        h('input', {
          type: 'text',
          value: tempAvatarUrl.value,
          onInput: (e) => { tempAvatarUrl.value = e.target.value },
    ...
    try {
      ...
      const result = await userStore.updateAvatar(tempAvatarUrl.value);
